...
class _FirstPage extends State<FirstPage> {
  ...
  List<bool> _isFilterVisible = List.generate(6, (_) => false); // 각 버튼의 상태를 관리하는 리스트
  ...
  void _toggleFilter(int index) {
    setState(() {
      for (int i = 0; i < _isFilterVisible.length; i++) {
        if (i == index) {
          _isFilterVisible[i] = !_isFilterVisible[i];
        } else {
          _isFilterVisible[i] = false;
        }
      }
    });
  }
            ...
            children: [
              ...
              for (int i = 0; i < detailed_filter.length; i++)
                AnimatedContainer(
                  ...
                  height: _isFilterVisible[i] ? MediaQuery.of(context).size.height * 1 / 2 : 0, // 컨테이너의 높이 조정
                  ...
                  child: SingleChildScrollView(
                    child: Column(
                      ...
                      children: [
                        // 세부 필터 버튼을 생성합니다.
                        for (int j = 0; j < 5; j++) // 세부 필터 버튼 예시
                          ElevatedButton(
                            onPressed: () {
                              print('${detailed_filter[i]} 필터의 옵션 $j 버튼이 눌렸습니다.');
                            },
                            child: Text('${detailed_filter[i]} 옵션 $j'),
                          ),
                      ],
                    ),
                  ),
